...
function startGame(level) {
		...
		for (let i = 1; i <= 2; i++) {
				const playerPromise = loadModel(world, 'models/object.gltf', new CANNON.Vec3(2 * i, debY, 5), 0.03, 1, `player-${i}`);
				// 0.02
				playersPromises.push(playerPromise);
		}

		for (let i = 1; i <= 3; i++) {
				const targetPromise = loadModel(world, 'models/object.gltf', new CANNON.Vec3(targets[level]['t' + i]['x'], targets[level]['t' + i]['y'], targets[level]['t' + i]['z']), 0.03, 1, 'target');
				// 0.02
